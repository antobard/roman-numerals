...
def roman_numerals_rules():
    """Ask for wich description of roman numerals"""
    description_choice = ""
    while description_choice != 3:
        ...
        if description_choice == 1:
            ...
            print("If a value placed before another is inferior to it, \
                it must be substracted to it")
            random_sub = choice(roman_num_special)
            print("For exemple : {} = {}"
                .format(random_sub[0], random_sub[1]))


def roman_num_quiz():
    """A quiz who asks to convert decimal / roman numerals"""
    ...
    while continue_game:
        ...
        while guess != str(roman_converter(random_number)):
            guess = input("Try to convert {} into {} notation : "
                .format(random_number, guess_type))
        print("Well Done !")
